...
class DiamondHeart {
  ...
  initAudioReactivity() {
    // Tạo một đối tượng Howl giả lập mà không có âm thanh thực
    this.sound = {
      play: function() {
        // Giả lập sự kiện onplay
        document.querySelector(".valentine-text").style.opacity = "1";
        document.querySelector(".loading-screen").style.opacity = "0";
      },
      fade: function() {
        // Giả lập hàm fade không làm gì cả
        return;
      }
    };

    // Tạo một analyser giả lập để tránh lỗi
    this.analyser = {
      fftSize: 2048,
      getByteFrequencyData: function(array) {
        // Điền mảng với dữ liệu giả lập
        for (let i = 0; i < array.length; i++) {
          array[i] = Math.random() * 50; // Giá trị ngẫu nhiên để tạo hiệu ứng
        }
      }
    };
    
    // Tạo dữ liệu âm thanh giả lập
